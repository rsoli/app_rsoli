...
export class FormularioGeocercaPage implements OnInit {
  ...
  constructor(
	...
	private geocerca_page:GeocercaPage
  ) { 
  ...
  ngOnInit() {
	...
	this.id_area=this.route.snapshot.params.id;
	// this.id_area = Number(this.route.snapshot.paramMap.get("id"));
    new L.Control.Draw(this.options);

	
	
	this.geocerca.id=Number(this.route.snapshot.params.id);
	this.geocerca.nombre_geocerca=this.route.snapshot.params.nombre_geocerca;
	this.geocerca.descripcion=this.route.snapshot.params.descripcion;
	this.geocerca.area=this.route.snapshot.params.area;
	this.geocerca.tipo_geocerca=this.route.snapshot.params.tipo_geocerca;
	this.tipo_area=this.route.snapshot.params.tipo_geocerca;

	this.area=this.route.snapshot.params.area; //aux

	console.log("llegoo ",this.geocerca.area);
  ...
  async GuardarGeocerca(){
	...
	if(JSON.parse(JSON.stringify(data)).mensaje[0]){
		...
		this.geocerca_page.GetGeocercas();
		this.router.navigate(['/geocerca']); 
